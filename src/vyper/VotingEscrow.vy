...
whitelisted_contracts: public(HashMap[address, bool])
...
def __init__(token_addr: address, _name: String[64], _symbol: String[32], _version: String[32]):
    ...
    @param token_addr JigsawToken address
...
@external
def whitelist_contract(addr: address):
    """
    @notice Set an external contract to check for approved smart contract wallets
    @param addr Address of Smart contract checker
    """
    assert msg.sender == self.admin,"1000"
    self.whitelisted_contracts[addr] = True

@external
def remove_whitelisted_contract(addr: address):
    """
    @notice Set an external contract to check for approved smart contract wallets
    @param addr Address of Smart contract checker
    """
    assert msg.sender == self.admin,"1000"
    self.whitelisted_contracts[addr] = False
...
def changeController(_newController: address):
    ...
    """
    assert msg.sender == self.controller,"1000"
...
def balanceOfAt(addr: address, _block: uint256) -> uint256:
    ...
    # reference yet
    assert _block <= block.number,"2001"
...
def deposit_for(_addr: address, _value: uint256):
    ...
    assert _value > 0,"2100"  # dev: need non-zero value
    assert _locked.amount > 0, "2001"
    assert _locked.end > block.timestamp, "3066"
...
def create_lock(_value: uint256, _unlock_time: uint256):
    ...
    assert _value > 0,"2100"  # dev: need non-zero value
    assert _locked.amount == 0, "2001"
    assert unlock_time > block.timestamp, "3066"
    assert unlock_time <= block.timestamp + MAXTIME,"3044"
...
def increase_unlock_time(_unlock_time: uint256):
    ...
    assert _locked.end > block.timestamp, "3061"
    assert _locked.amount > 0, "2001"
    assert unlock_time > _locked.end, "3066"
    assert unlock_time <= block.timestamp + MAXTIME, "3044"
...
def withdraw():
    ...
    """
    _locked: LockedBalance = self.locked[msg.sender]
    assert block.timestamp >= _locked.end, "3066"
