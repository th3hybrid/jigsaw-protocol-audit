...
def commit_admin(_addr: address):
    ...
    """
    assert msg.sender == self.admin,"1000"  # dev: access denied
    assert self.future_admin == ZERO_ADDRESS,"3000"
...
def recover_balance(_coin: address) -> bool:
    ...
    @return bool success
    """
    assert msg.sender == self.admin,"1000" #dev: !authorized
    assert _coin != self.token,"3000" #dev: protected token
...
def ve_for_at(_user: address, _timestamp: uint256) -> uint256:
    """
    @notice Get the veJigsawToken balance for `_user` at `_timestamp`
    @param _user Address to query balance for
    @param _timestamp Epoch time
    @return uint256 veJigsawToken balance
    """
    ve: address = self.voting_escrow
...
def queueNewRewards(_amount: uint256) -> bool:
    ...
    @return bool success
    """
    assert _amount > 0,"2100"
    assert not self.is_killed,"1200"
