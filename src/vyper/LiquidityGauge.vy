...
@external
def initialize(lp_addr: address, _minter: address, _admin: address):
    """
    @notice Initializes the cloned Liquidity Gauge
    @param lp_addr Liquidity Pool contract address
    @param _minter Minter contract address
    @param _admin Admin who can kill the gauge
    """
    assert not self.initialized, "3072"
    assert lp_addr != ZERO_ADDRESS,"3000"
    assert _minter != ZERO_ADDRESS,"3060"

    self.initialized = True

    _symbol: String[26] = ERC20Extended(lp_addr).symbol()
    self.name = concat("jigsaw.lol ", _symbol, " Gauge Deposit")
    self.symbol = concat(_symbol, "-gauge")

    self.lp_token = lp_addr
    self.minter = _minter
    jigsaw_addr: address = Minter(_minter).token()
    self.jigsaw_token = jigsaw_addr
    ...
    self.inflation_rate = JigsawToken20(jigsaw_addr).rate()
    self.future_epoch_time = JigsawToken20(jigsaw_addr).future_epoch_time_write()
    self.admin = _admin
...
def _update_liquidity_limit(addr: address, l: uint256, L: uint256):
    """
    @notice Calculate limits which depend on the amount of Jigsaw token per-user.
            Effectively it calculates working balances to apply amplification
            of Jigsaw production by Jigsaw
...
def _checkpoint(addr: address):
    ...
    @param addr User address
    """
    _token: address = self.jigsaw_token
    ...
    if prev_future_epoch >= _period_time:
        self.future_epoch_time = JigsawToken20(_token).future_epoch_time_write()
        new_rate = JigsawToken20(_token).rate()
...
@external
def kill_me():
    assert msg.sender == self.admin,"1000"
...
def commit_transfer_ownership(addr: address):
    ...
    """
    assert msg.sender == self.admin,"1000"  # dev: admin only
...
def apply_transfer_ownership():
    ...
    assert _admin != ZERO_ADDRESS,"3000"  # dev: admin not set
    self.admin = _admin
...
def user_checkpoint(addr: address) -> bool:
    ...
    @return bool success
    """
    assert (msg.sender == addr) or (msg.sender == self.minter),"1000"  # dev: unauthorized
...
def kick(addr: address):
    ...
    assert ERC20(self.voting_escrow).balanceOf(addr) == 0 or t_ve > t_last,"1000" # dev: kick not allowed
    assert self.working_balances[addr] > _balance * TOKENLESS_PRODUCTION / 100,"3066"  # dev: kick not needed
...
def deposit(_value: uint256, addr: address = msg.sender):
    ...
    """
    if addr != msg.sender:
        assert self.approved_to_deposit[msg.sender][addr],"1000"
    ...
    if _value != 0:
        ...
        assert ERC20(self.lp_token).transferFrom(msg.sender, self, _value),"3037"
...
def withdraw(_value: uint256):
    ...
    assert ERC20(self.lp_token).transfer(msg.sender, _value),"3037"
