...
token: public(address)  # Jigsaw token
...
def __init__(_token: address, _voting_escrow: address):
    ...
    """
    assert _token != ZERO_ADDRESS,"3000"
    assert _voting_escrow != ZERO_ADDRESS,"3000"
...
def add_gauge(addr: address, gauge_type: int128, weight: uint256):
    ...
    assert (gauge_type >= 0) and (gauge_type < self.n_gauge_types),"3066"
    assert self.gauge_types_[addr] == 0,"3017"  # dev: cannot add the same gauge twice
    ...
    prev: uint256 = self.total_gauges
    self.total_gauges = prev + 1
...
def vote_for_gauge_weights(_gauge_addr: address, _user_weight: uint256):
    ...
    next_time: uint256 = (block.timestamp + WEEK) / WEEK * WEEK
    assert lock_end > next_time,"2100"
    assert (_user_weight >= 0) and (_user_weight <= 10000),"2002"
    assert block.timestamp >= self.last_user_vote[msg.sender][_gauge_addr] + WEIGHT_VOTE_DELAY,"3067"

    gauge_type: int128 = self.gauge_types_[_gauge_addr] - 1
    assert gauge_type >= 0, "2001"
    ...
    assert (power_used >= 0) and (power_used <= 10000),"3066"
